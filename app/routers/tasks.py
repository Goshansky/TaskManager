...
@router.post(
    "/broken", response_model=TaskResponse, status_code=status.HTTP_201_CREATED
)
async def create_new_broken_task(
    task: BrokenTaskCreate, db: AsyncSession = Depends(get_db)
):
